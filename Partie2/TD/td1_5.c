/**
 * Programme calculant les valeurs d'une carte à joueur
 * 
 * fonction erreur laissée "au cas où", on voit ici l'interet de la
 * compilation separee
 *
 * historique: 
 ...
 *  MAJ      le 10/02/2021 - A. Habrard - correction de la détection d'erreur dans la 2eme fonction (card_name[1]!='\0')
...
/* ---- 
 * Fonction de calcul de valeur de carte
 * utilisant uniquement des caractères
 */
int calculer_carte_avec_caracteres_uniquement(char * card_name)
{
  int val=-1; 
  if (card_name[0] =='K' || card_name[0] =='Q' || card_name[0] =='J' || (card_name[0] =='1' && card_name[1] =='0')) {
    val = 10;
    ...
    /* en particulier le test card_name[1]!='\0' permet de détecter les mauvaises entrées comme 17 */
...
int main()
{
  ...
  int retour_scanf=0; /*variable de vérification du retour du scanf */
  ...
  do{
    ...
    if(strcmp(card_name,"N")) /* si c'est != 0, les 2 chaines sont differentes, on peut aussi utiliser card_name[0]!='N' */
      {
	/*calcul de la valeur*/
...
/* Note : dans le code ci-dessus, la fonction d'erreur arrête le programme dès qu'une entrée n'est pas correcte    */
/*        si vous voulez néanmoins vérifier les affichages des deux fonctions, vous pouvez commenter les fonctions */

/**
 * Programme impl�mentant une fonction qui affiche une chaine � l'envers
 ...
 *
 * Exemple d'utilisation de la biblioth�que assert
 *
 * historique: 
...
void affiche_inverse_ite(char *chaine) {
    ...
    /* on cherche le dernier caract�re, on pourrait aussi utiliser i=strlen(chaine), dans ce cas ne pas oublier d'inclure <string.h>*/
    while (chaine[i] != '\0') {
        i++;
    }
    i--;
...
/*fonction auxiliaire qui est en fait la fonction r�cursive */
void affiche_inverse_recu_aux(char *chaine, int index) {
    if (index >= 0) {
...
/* ---------- autre version sans besoin de faire appel � strlen -------*/
/*fonction recursive affichant l'inverse, on utilise une fonction auxiliaire ayant un argument de plus*/
void affiche_inverse_recu2(char *chaine) {

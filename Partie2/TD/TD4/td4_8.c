...
void recopie_liste_recu(liste l, liste * resultat)
{
  if(est_liste_vide(l)) //si la liste est vide, on a pour resultat une liste vide
    ...
    *resultat=insere_element_liste(liste_vide(),l->objet); //on cree un nouveau maillon avec l'objet courant et on le "met" dans resultat
    recopie_liste_recu(l->suivant,&((*resultat)->suivant)); //on copie la suite de la liste en mettant le début de la copie à (*resultat)->suivant
  }
}
